...
CREATE TABLE users (
    ...
    city_id INT REFERENCES cities(id)
            ...
            [
                ...
                'answer' => '**БКНФ** (Boyce-Codd Normal Form, **BCNF**) — **усиление 3НФ**.

**Формальное определение:** для каждой нетривиальной функциональной зависимости `X → Y` в таблице **`X` должен быть суперключом** (то есть значения `X` уникальны в таблице).

**Простыми словами:** **если `X` определяет `Y`, то `X` обязан быть уникальным**.

**3НФ vs БКНФ:**

| | **3НФ** | **БКНФ** |
|---|---|---|
| Транзитивных зависимостей | нет | нет |
| Все зависимости от **суперключа** | да, **с исключением** для «prime attributes» (атрибутов кандидатных ключей) | **строго все**, без исключений |
| Где расходятся | таблицы с **перекрывающимися кандидатными ключами** | то же — БКНФ режет жёстче |

**Классический пример нарушения БКНФ при 3НФ-соблюдении:**

Таблица `enrollments(student_id, course, teacher)`:
- предполагаем, что **один курс ведёт ровно один преподаватель** → `course → teacher`;
- кандидатные ключи: `(student_id, course)` и `(student_id, teacher)`;
- зависимость `course → teacher` — `course` **не является** суперключом → **нарушение БКНФ**;
- технически в 3НФ таблица может **числиться нормализованной** (исключение для prime attributes).

**Симптом:**
- если преподаватель сменился — обновлять **все строки** этого курса;
- если курса ещё нет в `enrollments` — преподаватель «не существует» в системе.

**Лечение:** разнести в две таблицы:
- `courses(course, teacher)` — справочник;
- `enrollments(student_id, course)` — записи студентов.

**Практика:** в **большинстве реальных схем** БКНФ **совпадает с 3НФ** — расхождения встречаются в учебных примерах с несколькими перекрывающимися ключами. На собеседовании достаточно понимать, что **БКНФ устраняет аномалии, которые 3НФ ещё пропускает**.',
                'difficulty' => 4,
                'topic' => 'database.normalization',
            ],
            [
