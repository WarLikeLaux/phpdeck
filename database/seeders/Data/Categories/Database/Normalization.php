...
CREATE TABLE student_course (
    ...
    PRIMARY KEY (student_id, course_id)
            ...
            [
                'category' => 'Базы данных',
                'question' => 'Что такое 1НФ (первая нормальная форма) простыми словами?',
                'answer' => 'Каждая ячейка таблицы содержит ОДНО неделимое значение. Нельзя: в колонке phones хранить «+7-111, +7-222, +7-333». Можно: одна строка на телефон в отдельной таблице phones, либо отдельные колонки phone1/phone2 (хуже). Атомарность данных — основа всех остальных НФ.',
                'difficulty' => 2,
                'topic' => 'database.normalization',
            ],
            [
                'category' => 'Базы данных',
                'question' => 'Что такое 2НФ (вторая нормальная форма) простыми словами?',
                'answer' => '1НФ + каждый не-ключевой атрибут зависит от ВСЕГО первичного ключа (а не только от его части). Актуально для составных PK. Если в таблице order_items с PK (order_id, product_id) есть колонка product_name — она зависит только от product_id, а не от пары → надо вынести в таблицу products.',
                'difficulty' => 3,
                'topic' => 'database.normalization',
            ],
            [
                'category' => 'Базы данных',
                'question' => 'Что такое 3НФ (третья нормальная форма) простыми словами?',
                'answer' => '2НФ + не должно быть транзитивных зависимостей: не-ключевой атрибут не должен зависеть от другого не-ключевого. В таблице users с колонками city, country — country зависит от city, а не от users.id → выносим в таблицу cities со связью country_id. Большинство реальных БД нормализуют до 3НФ, дальше редко.',
                'difficulty' => 3,
                'topic' => 'database.normalization',
            ],
        ];
