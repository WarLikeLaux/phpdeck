...
SELECT name FROM suppliers;',
            ...
            [
                ...
                'answer' => 'Подзапрос — это SELECT внутри другого запроса. По месту: в SELECT (скалярный подзапрос — возвращает одну строку/одну колонку, иначе ошибка), в FROM (производная таблица / inline view), в WHERE/HAVING (с IN, EXISTS, =, ANY, ALL). По связи: некоррелированный (выполняется один раз) и коррелированный (ссылается на колонки внешнего запроса, логически считается для каждой строки — оптимизатор может развернуть в JOIN). На читаемости часто выигрывает CTE (WITH ... AS), на производительности — JOIN или EXISTS вместо IN с большим списком. Скалярный подзапрос в SELECT удобен, но в OLTP-нагрузке может стать N+1 на стороне БД, если выбирается миллион строк.',
...
FROM users;',
            ...
            [
                ...
                'answer' => 'NULL — это маркер "значение неизвестно/отсутствует", а не ноль и не пустая строка. Любая арифметика и сравнение с NULL дают NULL: NULL = NULL → NULL (не TRUE!), NULL + 5 → NULL, NULL <> 1 → NULL. Поэтому в SQL действует трёхзначная логика: TRUE, FALSE, UNKNOWN. WHERE/JOIN ON пропускают только строки, где условие явно TRUE — все UNKNOWN отбрасываются. Для проверки на NULL есть отдельные операторы IS NULL / IS NOT NULL. Из той же логики растут грабли: NOT IN с подзапросом, который вернул NULL, "не находит" вообще ничего (TRUE становится UNKNOWN), поэтому NOT EXISTS безопаснее. COUNT(*) считает все строки, COUNT(col) — только не-NULL значения.',
...
SELECT COUNT(*) FILTER (WHERE status = \'completed\') AS done FROM orders;',
            ...
            [
                ...
                'answer' => 'IN сравнивает значение со списком значений или результатом подзапроса по колонке. EXISTS проверяет, вернул ли подзапрос хоть одну строку, и возвращает TRUE/FALSE — сам контент строки не важен (поэтому пишут SELECT 1 FROM ...). Исторически считалось, что EXISTS быстрее, но современные оптимизаторы (PostgreSQL 9+, MySQL 8+, SQL Server) транслируют и IN(subquery), и EXISTS(subquery) в один и тот же semi-join, план чаще всего идентичный. Принципиальная разница — в семантике NULL: NOT IN с NULL внутри подзапроса возвращает UNKNOWN для всех строк и "не находит" ничего; NOT EXISTS работает корректно. Поэтому в позитиве IN/EXISTS взаимозаменяемы, в негативе всегда выбирают NOT EXISTS.',
                'code_example' => '-- IN
...
INNER JOIN orders o ON o.user_id = u.id;',
            ...
            [
                ...
                'answer' => 'Для INNER JOIN это семантически эквивалентно — оптимизатор свободно перемещает предикаты между ON и WHERE. Для LEFT JOIN расположение принципиально. Условие в ON применяется ВО ВРЕМЯ соединения: оно решает, какие строки правой таблицы прицепляются, а строки левой таблицы остаются все, и если совпадения нет — справа подставляются NULL. То же условие в WHERE применяется уже ПОСЛЕ соединения — к итоговому набору, и любая проверка правой таблицы (кроме IS NULL) отбрасывает несовпавшие строки, превращая LEFT JOIN в INNER. Классическая ловушка: "все пользователи и их paid-заказы" — фильтр paid в WHERE убирает пользователей без paid-заказов целиком. Правильно класть условие на правую таблицу в ON, а в WHERE оставлять условия на левую.',
...
WHERE o.id IS NULL; -- пользователи без заказов',
            ...
            [
                ...
                'answer' => 'LIKE поддерживает всего два метасимвола: % (любая последовательность символов, включая пустую) и _ (один символ). REGEXP (синоним RLIKE) принимает полноценные регулярные выражения с якорями ^ $, классами [a-z], альтернативами | и квантификаторами + * {n}. REGEXP мощнее по выразительности, но почти всегда вызывает полное сканирование таблицы — обычный B-tree индекс по колонке помогает только LIKE и только при префиксном шаблоне без % в начале (LIKE \'abc%\'). LIKE \'%abc%\' и любой REGEXP оптимизатор отдаст в Seq Scan. Поэтому REGEXP уместен для разовых отчётов и админ-скриптов, а для частых поисков нужны FULLTEXT-индекс, n-gram индекс, отдельный поисковой движок (Elasticsearch, Meilisearch) или предвычисленные нормализованные колонки.',
...
FROM orders;',
            ...
            [
                ...
                'answer' => 'Сравнение через ANY (синоним SOME) истинно, если условие верно хотя бы для одного значения из подзапроса: x > ANY(SELECT ...) равносильно x > MIN(...). ALL требует, чтобы условие выполнялось для всех возвращённых значений: x > ALL(SELECT ...) то же, что x > MAX(...). На пустом подзапросе семантика часто удивляет: ANY всегда даёт FALSE (нет ни одного значения, для которого условие верно), а ALL — TRUE (по логике "для всех нуля значений верно что угодно"). NULL внутри подзапроса тоже даёт UNKNOWN и ломает результат. На практике эти конструкции встречаются редко — почти всегда их переписывают через MIN/MAX в скалярном подзапросе или через EXISTS, потому что план получается такой же, а читается понятнее.',
...
QUALIFY rn <= 3; -- или WHERE в подзапросе',
            ...
            [
                ...
                'answer' => 'UPSERT — "INSERT, а если строка с таким ключом уже есть — UPDATE", выполняемый одной атомарной командой. Нужен, чтобы не делать вручную SELECT → INSERT/UPDATE: между двумя запросами возможна гонка и появится дубль или ошибка уникальности. В MySQL: INSERT ... ON DUPLICATE KEY UPDATE — срабатывает при конфликте по PRIMARY KEY или любому UNIQUE-индексу, в SET-части доступна функция VALUES(col) или новый алиас VALUES AS new для ссылки на вставляемое значение. В PostgreSQL — стандартный синтаксис SQL:2003: INSERT ... ON CONFLICT (col) DO UPDATE SET ... (через псевдотаблицу EXCLUDED достаём вставляемые значения), либо ON CONFLICT DO NOTHING, чтобы тихо пропустить дубль. Конфликт ловится только по указанному столбцу/индексу, поэтому в PG нужно явно назвать колонку с UNIQUE.',
                'code_example' => '-- MySQL
