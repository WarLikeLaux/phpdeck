    ...
    public static function all(): array
    {
        return [
            [
                ...
                'answer' => 'Индекс — это вспомогательная структура данных, которая ускоряет поиск по таблице. Аналогия: толстая книга. Чтобы найти главу «Пингвины», ты не листаешь каждую страницу — ты смотришь в алфавитный указатель в конце книги, находишь «Пингвины — стр. 524» и сразу открываешь нужную страницу. Индекс в БД работает так же: вместо полного сканирования таблицы СУБД сначала идёт в индекс, находит позицию и читает только нужные строки. Минусы: индекс занимает место на диске и замедляет INSERT/UPDATE/DELETE (потому что вместе с данными надо обновлять и индекс). Поэтому индексы ставят на те колонки, по которым часто фильтруют (WHERE), соединяют (JOIN) и сортируют (ORDER BY) — но не на всё подряд.',
