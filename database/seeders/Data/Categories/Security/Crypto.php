...
\$checksum = hash_file('sha256', '/var/release.zip');",
            ...
            [
                'category' => 'Безопасность',
                'question' => 'Чем pepper (пеппер) отличается от соли и зачем его добавляют поверх password_hash()?',
...
Или через `sodium_crypto_pwhash_str` с дополнительным секретным ключом. Важно: `HMAC` делать **до** `password_hash`, а не после — иначе нарушается логика работы хэш-функции.

**Минусы pepper:**
- **ротация** — смена pepper = пересчёт **всех** хешей. Решается **версионированием** (`pepper_v2` + `pepper_v1` параллельно)
- **`bcrypt` лимит 72 байта** — `HMAC-SHA256` даёт 32 байта в hex (64 символа) — впритык. Лучше использовать `argon2id`.
- **сложность** — больше кода, больше шансов ошибиться в реализации.

**Когда стоит:** банки, финтех, критичные данные с высоким риском утечки БД. Большинству проектов **хватает обычного `password_hash()`** с `argon2id`.',
                'difficulty' => 4,
                'topic' => 'security.crypto',
            ],
            [
