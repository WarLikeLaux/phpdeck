    ...
    public static function all(): array
    {
        return [
            [
                ...
                'answer' => 'Хеширование — ОДНОСТОРОННЕЕ преобразование: из «password123» получаешь строку фиксированной длины, и обратно по этой строке пароль не достать (никаким ключом). Применяется там, где значение знать не надо, надо только сверить: пароли, контрольные суммы файлов, цифровые отпечатки. Шифрование — ДВУСТОРОННЕЕ: зашифровал ключом — расшифровал тем же (или парным) ключом, получил исходные данные. Применяется там, где значение нужно потом прочитать: номера карт в БД, токены, личные данные, переписка. Главное правило джуна: пароли пользователей — ХЕШИРУЕМ (password_hash), личные данные и секреты — ШИФРУЕМ (Crypt::encrypt в Laravel, openssl_encrypt в чистом PHP). Если код «расшифровывает пароль» — это баг, пароли не должны быть расшифровываемыми в принципе.',
                'code_example' => "<?php
...
if (hash_equals(\$expected, \$received)) { /* ok */ }",
            ...
            [
                ...
                'answer' => 'HTTPS = HTTP поверх TLS. Даёт три гарантии сразу: 1) Шифрование — провайдер, владелец Wi-Fi в кафе или начальник через корпоративный прокси не прочитают трафик (пароли, токены, куки уходят в виде шифра). 2) Целостность — никто в пути не подменит ответ: не вставит рекламный JS, не модифицирует JSON. 3) Аутентичность — TLS-сертификат, подписанный доверенным центром (Let\'s Encrypt, DigiCert), подтверждает, что ты говоришь именно с example.com, а не с поддельным сервером, прикинувшимся им. Без HTTPS любая публичная Wi-Fi-сеть — угроза: атакующий поднимает точку «Free_Airport_WiFi», и весь HTTP-трафик у него как на ладони. В 2026 HTTPS обязателен везде: браузеры пишут «Не защищено» на HTTP-страницах, многие API возвращают ошибку, поисковики понижают в выдаче. Сертификат бесплатно даёт Let\'s Encrypt, всё настраивается одной командой certbot.',
