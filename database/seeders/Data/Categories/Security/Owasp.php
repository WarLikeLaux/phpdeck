...
Большая часть реальных взломов идёт именно через эти классы — поэтому `OWASP Top 10` учат все разработчики.',
            ...
            [
                ...
                'answer' => '**№1** в `OWASP Top 10 2021` — ошибки в проверках «**кому что можно**».

**Типичные баги**:

1. `GET /users/42/orders` — юзер `1` видит заказы юзера `42`, нет проверки владельца (`IDOR`).
2. Админская кнопка спрятана только на фронте, а `POST /admin/users` открыт публично.
3. **Mass assignment** `role=admin` через `update($request->all())`.
4. Прямой доступ к `/admin` без middleware.
5. Можно поменять `user_id` в теле запроса и записать данные другому юзеру.

**Принципы защиты**:

- **deny by default** — закрыто всё, что не открыто явно;
- проверка авторизации на **КАЖДОМ** эндпоинте (не только в UI);
- `Policies`/`Gates` в Laravel + middleware `can:`;
- явный whitelist полей в `$fillable` / `FormRequest`.',
                'code_example' => "<?php
...
public function update(UpdatePostRequest \$request, Post \$post) {
    ...
    \$post->update(\$request->validated());
            ...
            [
                ...
                'answer' => '**`Insecure Direct Object Reference`** — частный случай `Broken Access Control`.

Пользователь меняет `ID` в URL/теле запроса и получает доступ к **чужим** данным:

- `/invoices/123` → `/invoices/124` покажет чужой инвойс;
- если сервер делает `Invoice::find($id)` без проверки владельца — данные утекают.

**Защита**:

- **авторизация на каждом** обращении к ресурсу;
- фильтр по `auth()->id()` прямо в запросе — `auth()->user()->invoices()->findOrFail($id)`;
- или `Gate`/`Policy` в Laravel — `$this->authorize(\'view\', $invoice)`.

**Дополнительно**: `UUID`/`ULID` вместо инкрементных `id`, чтобы соседний id нельзя было угадать перебором. Это **security-through-obscurity**, не основная защита — без проверки прав всё равно дыра.',
                'code_example' => "<?php
...
public function show(Invoice \$invoice) {
    ...
    return \$invoice;
            ...
            [
                ...
                'answer' => '**№2** в `OWASP Top 10 2021` (раньше назывался `Sensitive Data Exposure`) — ошибки в работе с криптографией и чувствительными данными.

**Примеры**:

- пароли в `md5`/`sha1` или вообще в открытом виде;
- номера карт, паспорта, ПДн в БД **без шифрования**;
- отсутствие `HTTPS` — пароли и токены ходят в открытую;
- устаревшие алгоритмы: `DES`, `RC4`, `SHA-1`, `MD5`;
- **самописное** шифрование («я придумал свой алгоритм»);
- захардкоженные ключи в коде.

**Защита**:

- пароли — `bcrypt`/`argon2id` через `password_hash`/`Hash::make`;
- чувствительные данные — `AES-GCM` через `Crypt::encrypt` или `openssl_encrypt`;
- **готовые** библиотеки (`libsodium`, openssl), не своя реализация;
- `HTTPS` везде + `HSTS`.',
                'difficulty' => 2,
                'topic' => 'security.owasp',
            ],
            [
                ...
                'answer' => 'Класс уязвимостей, в которых атакующий подсовывает свой **код** туда, где сервер ждёт **данные**.

**Корень проблемы** — **конкатенация** пользовательского ввода со строкой команды/запроса.

**Виды**:

- `SQL Injection` — вставка SQL в запрос;
- `Command Injection` — вставка shell-команд в `exec`/`system`;
- `LDAP`/`NoSQL Injection` — в запросы к соответствующим БД;
- `XSS` — инъекция JS в HTML (тоже injection, но в браузер жертвы);
- `Template Injection` — `Blade`/`Twig`/`Jinja` через пользовательский ввод;
- `Header Injection` — `\\r\\n` в HTTP-заголовках.

**Общее правило защиты**:

- **НЕ склеивать** ввод с командой строкой;
- использовать **параметризованные API**: `prepared statements` (`PDO`, Eloquent), `escapeshellarg`, `htmlspecialchars`;
- **валидация по whitelist** (разрешённые значения), а не по blacklist (запрещённые).',
                'difficulty' => 2,
                'topic' => 'security.owasp',
            ],
            [
...
public function verifyReset(Request \$r) {
    ...
    Auth::login(\$record->user);
            ...
            [
                ...
                'answer' => 'Слабые / дефолтные настройки безопасности. Самый **дешёвый** класс взломов — кода атакующего может вообще не понадобиться.

**Типичные грабли в Laravel/PHP-стеке**:

- `APP_DEBUG=true` на проде — stack trace + переменные окружения наружу;
- **дефолтные** пароли (`admin`/`admin`, root БД без пароля);
- открытые наружу порты `MySQL`/`Redis`/`Memcached`;
- `CORS` с `Access-Control-Allow-Origin: *` + `credentials: true`;
- отсутствие `HTTPS` / `HSTS`;
- доступный `/phpinfo.php`, `/.env`, `/.git/config`;
- незакрытые админки CI/мониторинга (`/horizon`, `/telescope` без auth).

**Защита**:

- **чек-лист** на деплое;
- security headers — `CSP`, `HSTS`, `X-Frame-Options`, `X-Content-Type-Options`;
- сканеры конфигов (`composer audit`, `enlightn`);
- отдельные `prod`-настройки, **минимум прав** у пользователей БД.',
                'difficulty' => 2,
                'topic' => 'security.owasp',
            ],
            [
                ...
                'answer' => 'Использование **старых версий** библиотек, фреймворков или рантайма с **публично известными** уязвимостями (`CVE`).

**Хрестоматийный пример** — `Log4Shell` в `Log4j` (`CVE-2021-44228`): любая Java-программа с уязвимой версией Log4j позволяла `RCE` через специальную строку в логе. Аналогично в PHP-экосистеме регулярно прилетают `CVE` в `Laravel`, `Symfony`, `Guzzle`.

**Защита**:

- **`composer audit`** / `npm audit` в CI — провалить билд при known CVE;
...
npm audit fix",
            ...
            [
                ...
                'answer' => 'Слабые места в **логине** и **сессиях** (раньше — `Broken Authentication`, №7 в `OWASP Top 10 2021`).

**Примеры**:

- разрешены пароли вида `123456`, `qwerty`, `password`;
- нет защиты от **brute force** (`rate limit` / `lockout`);
- `session ID` в URL (`?PHPSESSID=...`) — утекает в `Referer`/логи;
- сессия **не инвалидируется** после `logout` или смены пароля;
- утечка «пользователь не существует» в форме входа (разные сообщения для неверного email vs пароля);
- **MFA отсутствует** для админов;
- `session ID` не регенерируется после логина (session fixation).

**Защита**:

- `rate limit` на `/login`, `2FA` (хотя бы для админов);
- требования к паролям: **длина** важнее, чем сложность; проверка по `haveibeenpwned`;
- регенерация `session ID` после логина и смены пароля;
- одинаковый ответ при невалидных credentials.',
                'difficulty' => 2,
                'topic' => 'security.owasp',
            ],
            [
...
//   crossorigin=\"anonymous\"></script>",
            ...
            [
                ...
                'answer' => 'Атака есть, а ты о ней **не узнаешь** — №9 в `OWASP Top 10 2021`. По статистике инциденты в среднем обнаруживаются через **месяцы**, и виновата как раз эта проблема.

**Симптомы**:

- **не логируются** неудачные логины и попытки авторизации;
- в логах нет `user_id` / `IP` / `request-id`;
- нет **алертов** на массовый `403`/`500`/неудачные логины;
- логи **стираются раньше**, чем заметили инцидент;
- логи лежат только на одной машине и не реплицируются.

**Защита**:

- централизованный лог-стек: `ELK`/`Loki`, `Sentry`, `Datadog`, `Grafana`;
- **алерты** на аномалии (`>100` неудачных логинов в минуту, всплеск `5xx`);
- хранение `access`-логов **90+ дней**;
- периодический ручной просмотр (security audit);
- маскирование секретов в логах (пароли, токены).',
                'difficulty' => 2,
                'topic' => 'security.owasp',
            ],
            [
