    ...
    public static function all(): array
    {
        return [
            [
                ...
                'answer' => 'Аутентификация (authentication, AuthN) — «кто ты?», проверка личности: логин+пароль, токен, отпечаток пальца, SSO через Google. Авторизация (authorization, AuthZ) — «что тебе можно?», проверка прав на конкретное действие: может ли этот юзер редактировать ИМЕННО этот пост, удалить ИМЕННО этот заказ. Порядок всегда такой: сначала аутентификация (узнали юзера), потом авторизация (проверили его права). Часто путают из-за похожих слов, но это разные шаги — в Laravel за первое отвечает guard (Auth::check), за второе — Policy/Gate ($this->authorize).',
                'code_example' => "<?php
...
// Route::put('/posts/{post}', ...)->middleware(['auth', 'can:update,post']);",
            ...
            [
                ...
                'answer' => 'Two/Multi-Factor Authentication — вход не только по паролю, а по двум независимым факторам. Три категории факторов: 1) Что ты знаешь (пароль, PIN). 2) Что у тебя есть (телефон с кодом из Google Authenticator/SMS, аппаратный ключ YubiKey). 3) Кто ты (отпечаток, Face ID). 2FA = пароль + один второй фактор. Зачем: если пароль утёк через фишинг или утечку базы, атакующий всё равно не войдёт без второго фактора. Самый распространённый второй фактор — TOTP (6-значный код, меняется каждые 30 секунд в приложении). SMS считается слабым (SIM swap), аппаратные ключи — самые надёжные.',
...
session(['2fa_passed' => true]);",
            ...
            [
                ...
                'answer' => 'НЕЛЬЗЯ: открытым текстом, через md5/sha1/sha256/base64. Они БЫСТРЫЕ — GPU-ферма перебирает миллиарды вариантов в секунду и пробивает «password123» за минуты. Если БД сольётся, все пароли считаются скомпрометированными. ПРАВИЛЬНО: password_hash($pass, PASSWORD_DEFAULT) — внутри bcrypt (или argon2id в новых версиях), который специально МЕДЛЕННЫЙ и сам кладёт случайную соль внутрь итоговой строки. Проверка через password_verify($input, $hashFromDb), оно само вытащит соль и cost из хеша. Никогда не пиши своё хеширование и не «улучшай» его солью+перцем вручную. Колонка в БД — VARCHAR(255), пароль с фронта приходит как есть, хешируется на сервере перед сохранением.',
                'code_example' => "<?php
