    ...
    public function pay(float $amount): bool
    {
        ...
        return true;
            ...
            [
                ...
                'answer' => 'Да. В PHP (как в Java/C#/C++) модификаторы видимости работают на уровне КЛАССА, а не экземпляра. private значит «доступно изнутри своего класса», а не «только своему $this». Это нужно для операций над двумя объектами одного типа: equals(self $other), Money::add(Money $other) — там $other->amount доступен. Короткая формулировка для собеса: «private — per-class, не per-instance».',
                'code_example' => '<?php
final class Money
{
...
// echo $a->amount; // Error: Cannot access private property',
            ...
            [
                ...
                'answer' => 'Абстракция — про дизайн интерфейса: выделить существенные операции и скрыть сложность, отвечает «что объект умеет». Инкапсуляция — про защиту состояния: спрятать данные за private и публиковать методы, гарантирующие инварианты, отвечает «как объект себя защищает». Абстракция выражается интерфейсами и абстрактными классами; инкапсуляция — модификаторами видимости и геттерами/сеттерами. Они дополняют друг друга: без инкапсуляции абстракция протекает, а без абстракции инкапсуляция превращается в анемичный набор геттеров.',
                'difficulty' => 2,
                'topic' => 'oop.four_principles',
                'code_example' => '<?php
