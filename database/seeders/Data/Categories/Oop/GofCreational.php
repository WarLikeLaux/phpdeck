...
echo $original->author->name; // Иван (благодаря __clone)',
            ...
            [
                ...
                'answer' => 'Factory отвечает за конструирование объекта: он знает, какие конкретные классы выбрать, какие параметры собрать, и инкапсулирует логику создания. Dependency Injection отвечает за доставку уже готовой зависимости тому, кто её использует, через конструктор или сеттер. Они не взаимоисключающие: фабрика часто сама регистрируется как сервис в DI-контейнере и инжектится туда, где нужно создавать объекты по требованию (например, по типу из запроса). DI решает "кто и как получает зависимость", фабрика — "как именно она создаётся".',
                'difficulty' => 3,
                'topic' => 'oop.gof_creational',
            ],
        ];
