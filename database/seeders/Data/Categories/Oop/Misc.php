    ...
    public function create(Request $r): void
    {
        ...
        $this->service->create($dto);
            ...
            [
                ...
                'answer' => '**Правила** изменения типов в **переопределённом методе наследника** (PHP 7.4+). Существуют ради **LSP**: код, написанный против родителя, должен **корректно работать** с любым подклассом.

**Три вариантности:**

| Правило | Где применяется | Направление | Пример |
|---|---|---|---|
| **Covariance** (ковариантность) | **возвращаемый тип** | можно **СУЗИТЬ** | `Animal` → `Dog` |
| **Contravariance** (контравариантность) | **тип параметра** | можно **РАСШИРИТЬ** | `Dog` → `Animal` |
| **Invariance** (инвариантность) | **типы свойств** | **менять нельзя** | `Fatal error` |

**Почему так — интуитивно:**

- **Возврат сужаем** — клиент ждёт `Animal`, получает `Dog`. `Dog` — это `Animal`, всё ок.
- **Параметр расширяем** — родитель принимал `Dog`, наследник принимает любое `Animal`. Клиент даёт `Dog` — наследник его примет.
- **Свойство** читается **и** записывается → должно остаться **точно тем же типом**, иначе одно из направлений сломается.

**Особенности PHP:**

- Раньше PHP **запрещал** ковариантность; с **7.4** разрешена.
- **Контравариантность параметров** — с **7.2** (раньше требовалось полное совпадение).
- Для **свойств** обходного пути нет — используй **геттер/сеттер** (методы поддерживают вариантность) вместо публичного поля.
- **Скаляры тоже** участвуют: `iterable` → `array`, `mixed` → `int`.',
                'code_example' => '<?php
class Animal {}
    ...
    public static function fromDollars(float $dollars): self
    {
        return new self((int)($dollars * 100), \'USD\');
            ...
            [
                ...
                'answer' => '**Open recursion** — когда метод **родителя** через `$this` вызывает **другой метод** того же объекта, а **потомок переопределяет** этот метод.

**Гибко** — позволяет «hook»-методам наследников встраиваться в алгоритм родителя. **Но опасно** — создаёт **fragile base class problem**.

**Fragile base class problem:**

| Симптом | Что происходит |
|---|---|
| **Невидимая связь** | родитель полагается на конкретное поведение `$this->add()`, но компилятор это не выражает |
| **Изменение в родителе ломает потомков** | оптимизировали `addMany()` (вызвали `$this->count += sum` вместо цикла с `$this->add()`) — логирующий потомок **потерял** логи |
| **Никто не предупреждён** | бинарной совместимости нет — наследник падает без диагностики |
| **Тестов недостаточно** | мокать `$this` нельзя |

**Защита:**

1. **`final` на хук-методах** — те, которые **не должны** переопределяться, помечать `final`. Расширяемость — только через явные template-methods.
2. **Композиция через Strategy** вместо наследования — явный контракт интерфейса, без скрытых вызовов через `$this`.
3. **Документировать «protected API»** — `@inheritDoc`, чёткое описание, какие методы хук-методы и в каком порядке вызываются.
4. **`#[\\Override]`** (PHP 8.3) на стороне потомка — фиксирует намерение и ловит опечатки при переименовании в родителе.

**Совет:** **наследование — extension point**, к нему **высокие требования**. Если не готов поддерживать обратную совместимость хуков — закрывай класс `final`.',
                'code_example' => '<?php
class Counter
{
    private int $count = 0;
...
- **YAGNI (You Aren\'t Gonna Need It)** — не пиши код «на будущее» в надежде, что пригодится. Чаще всего не пригодится, а лишний код придётся поддерживать.',
            ...
            [
                ...
                'answer' => '**Cohesion (сплочённость)** — насколько элементы внутри одного модуля **связаны общей целью**.

- **Высокая cohesion** — класс делает **одно дело** и делает его хорошо. **Цель.**
- **Низкая cohesion** — класс мешает **разнородные** функции в кучу. **Запах.**

**Семь уровней (от худшего к лучшему):**

| Уровень | Что | Пример |
|---|---|---|
| **Coincidental** | случайно вместе | `Utility` со всеми helper-методами |
| **Logical** | функции одной категории | «Все валидаторы в одном классе» |
| **Temporal** | вызываются в одно время | `Bootstrap::initAll()` — БД, кеш, логгер |
| **Procedural** | выполняются по очереди | `processOrder()` — шаги без общих данных |
| **Communicational** | работают с одними данными | `Report` — методы над одним dataset |
| **Sequential** | выход одного = вход другого | `Pipeline` — `parse` → `validate` → `save` |
| **Functional** | всё ради одной задачи | `TaxCalculator` — **идеал** |

**Связи с другими принципами:**

- **Высокая cohesion = SRP** — два названия одного.
- **Идёт в паре с low coupling** — узко и независимо.
- **Признак запаха** — слово «**Utility**», «**Helper**», «**Manager**» в названии часто значит **coincidental** cohesion.',
                'code_example' => '<?php
...
class TaxCalculator
{
    ...
    public function totalWithTax(Money $base, Country $c): Money {}
            ...
            [
                ...
                'answer' => '**Coupling (зацепление, связанность)** — насколько модули **зависят друг от друга**. **Низкая связанность — цель.**

**Шесть уровней (от худшего к лучшему):**

| Уровень | Что | Пример |
|---|---|---|
| **Content** | модуль лезет **внутрь** другого | `$other->privateField = ...` через reflection |
| **Common** | общие **глобальные** данные | global `$config`, статический `Cache::$store` |
| **Control** | один **управляет логикой** другого через флаги | `build($data, bool $asPdf)` — типичный bool-flag запах |
| **Stamp** | передают **сложные структуры**, используют часть | `notify(User $u)`, но нужен только `$u->email` |
| **Data** | передают **только нужные** параметры | `notify(string $email)` |
| **Message** | взаимодействие **через интерфейсы** | `notify(EmailRecipient $r)` — **идеал** |

**Связи с другими принципами:**

- **Low coupling = DIP + ISP + Инкапсуляция + Law of Demeter** в действии.
- Чем **уже интерфейс** взаимодействия — тем ниже coupling.
- **Stamp coupling** часто лечится через **ISP** — узкий интерфейс вместо «всего объекта».
- **Control coupling** лечится через **Strategy** — вместо bool-флага отдельный класс на каждую ветвь.

**Цель:** **High cohesion + low coupling** — узкая ответственность внутри, минимум связей наружу.',
                'code_example' => '<?php
...
final class ServiceLocator {
    ...
    public function get(string $name): object { return ($this->factories[$name])($this); }
            ...
            [
                ...
                'answer' => '**GRASP (General Responsibility Assignment Software Patterns)** — набор из **9 принципов** распределения **ответственностей** между классами. Автор — **Craig Larman**.

**9 принципов GRASP:**

| Принцип | Суть |
|---|---|
| **Information Expert** | ответственность — у того, у кого **данные** для её выполнения |
| **Creator** | объект создаёт другой, если **тесно с ним связан** (содержит, агрегирует) |
| **Controller** | use case **обрабатывает** не Entity, а специальный controller-объект |
| **Low Coupling** | минимизировать связи между классами |
| **High Cohesion** | внутри класса — связанные ответственности |
| **Polymorphism** | подменяй варианты через **полиморфизм**, не `switch` |
| **Pure Fabrication** | «искусственный» класс ради чистоты дизайна (`Repository`, `Mapper`) |
| **Indirection** | ввести посредника, чтобы развязать классы (Mediator, Adapter) |
| **Protected Variations** | стабильный **интерфейс** изолирует изменчивые места |

**GRASP vs SOLID:**

| | **SOLID** | **GRASP** |
|---|---|---|
| Вопрос | «**какими** должны быть классы?» | «**какому** классу отдать обязанность?» |
| Уровень | принципы качества | паттерны распределения |
| Когда применяешь | при оценке существующего класса | при проектировании нового |

**Не конкурируют — дополняют:** SRP × Information Expert, OCP × Polymorphism, DIP × Protected Variations.',
                'difficulty' => 4,
                'topic' => 'oop.misc',
            ],
            [
