    ...
    public static function all(): array
    {
        return [
            [
                ...
                'answer' => 'Adapter преобразует интерфейс одного класса в интерфейс, который ожидает клиент. Простыми словами — переходник для несовместимых API. Используется когда: 1) Интегрируешь сторонний SDK (Stripe, AWS), сигнатуры которого не вписываются в твой домен — оборачиваешь в AdapterImplements PaymentGateway. 2) Подключаешь legacy-класс к новому коду без правки старого. 3) Делаешь несколько разных реализаций (Stripe, PayPal, YandexKassa) совместимыми с одним интерфейсом приложения. Не путать с Facade (тот упрощает СЛОЖНУЮ подсистему за одним API) и Decorator (тот добавляет поведение, сохраняя интерфейс). Адаптер — МЕНЯЕТ интерфейс, ничего нового не добавляя.',
                'code_example' => '<?php
...
$c = new Circle(new Red());',
            ...
            [
                ...
                'answer' => 'Composite группирует объекты в ДРЕВОВИДНУЮ структуру и позволяет работать с группой так же, как с одиночным объектом. Ключевая идея: лист (File) и контейнер (Folder) реализуют ОДИН интерфейс (Component). Контейнер хранит детей и делегирует им вызов, рекурсивно агрегируя результат. Клиент не различает «один объект это или дерево». Применения: файловая система (size, search), DOM/UI-дерево (render), организационная структура (общая зарплата по отделу), пайплайны валидаторов. Подводный камень: метод add/remove на интерфейсе ломает безопасность (лист его «не умеет»), поэтому либо держать его только на контейнере, либо в листе бросать исключение — это компромисс «прозрачность vs безопасность».',
                'code_example' => '<?php
interface FsNode
{
