    ...
    public static function all(): array
    {
        return [
            [
                ...
                'answer' => '**Главное правило:** **сетка безопасности → маленькие шаги → тесты после каждого**. Не «переписать заново» — рефакторинг = поведение **не меняется**.

**Пошаговый процесс:**

1. **Накрыть текущее поведение тестами** — характеристические тесты (Michael Feathers): фиксируют то, **как сейчас работает**, даже если кривo. Без них любой рефакторинг — лотерея.
2. **Выделить ответственности по SRP** — отдельные классы: валидация, доступ к БД, рассылка, расчёт. Каждая — на свой класс.
3. **Вынести зависимости через DI** — `new SmtpMailer()` внутри → `__construct(Mailer $mailer)`. Это разрывает связанность и **открывает класс для тестов**.
4. **Применить паттерны** где видны типовые формы:
   - **Strategy** — вместо `switch` по типу.
   - **Factory** — вместо разветвлённой логики создания.
   - **Service-объект** — вместо «god method» в контроллере.
5. **Двигаться маленькими шагами** — после каждого extract method / extract class **прогон тестов**. Коммит после каждого зелёного шага.

**Чего НЕ делать:**

| Анти-практика | Почему опасно |
|---|---|
| **«Перепишу с нуля»** | потеря знаний, регрессии, бесконечный срок |
| Большие refactor-PR | невозможно ревьюить, конфликты с master |
| Рефакторить **без тестов** | потеряешь скрытое поведение, на которое полагаются клиенты |
| Менять имена + структуру **в одном коммите** | diff нечитаем |

**Инструменты IDE:** Extract Method / Extract Class / Inline / Rename — **выполнять автоматически**, не руками.',
                'difficulty' => 4,
