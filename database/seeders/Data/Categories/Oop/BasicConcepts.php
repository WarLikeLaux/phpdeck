    ...
    public static function all(): array
    {
        return [
            [
                ...
                'answer' => 'ООП (объектно-ориентированное программирование) — парадигма, в которой программа моделируется как набор взаимодействующих объектов. Каждый объект имеет состояние (свойства — данные) и поведение (методы — действия). Базовые принципы: инкапсуляция (прячем детали реализации), наследование (один класс может расширять другой), полиморфизм (один интерфейс — разные реализации), абстракция (выделяем главное, опускаем мелочи). Помогает структурировать большие программы, переиспользовать код и моделировать предметную область.',
                'code_example' => '<?php
...
echo $calc->add(2, 3); // 5',
            ...
            [
                ...
                'answer' => 'Свойство (property, поле) — это переменная, объявленная внутри класса. Свойство хранит данные конкретного объекта: у каждого объекта свой набор значений, но имена и типы свойств описаны в классе. С PHP 7.4 свойствам можно указать тип (public string $name), можно задать значение по умолчанию. С PHP 8.1 — пометить readonly (запись только из конструктора). Доступ снаружи — через объект и стрелку: $user->name.',
                'code_example' => '<?php
class Product
{
    public string $name;
    ...
    public function save(): void
    {
        ...
        error_log(\'saved at \' . time()); // потом своё
            ...
            [
                ...
                'answer' => 'Неизменяемое значение, привязанное к классу. Объявляется через const, доступ через :: и имя класса (Status::ACTIVE). Перед именем нет $ — это не переменная. По соглашению пишут UPPER_CASE. Удобно для перечислений статусов, кодов ошибок, версий API. Константы можно объявлять и внутри интерфейсов. С PHP 8.1 константу можно пометить final (потомок не сможет переопределить), с PHP 8.3 — указать тип.',
                'difficulty' => 1,
                'topic' => 'oop.basic_concepts',
                'code_example' => '<?php
class Status
    ...
    public function setName(string $name): void
    {
        ...
        $this->name = $name;
            ...
            [
                ...
                'answer' => 'Когда один класс (потомок) получает все public/protected свойства и методы другого класса (родителя). В PHP объявляется через extends: class Admin extends User. Потомок может: 1) добавить свои методы; 2) переопределить родительские (override); 3) использовать родительские без изменений. PHP разрешает наследовать только ОДИН класс. Для нескольких контрактов — implements (интерфейсы), для шаринга методов — use (трейты). Применяется, когда между классами есть отношение "является" (is-a): Admin — это User.',
                'code_example' => '<?php
class User
{
...
function run(Logger $log): void { $log->log(\'work\'); }',
            ...
            [
                ...
                'answer' => 'Указание ожидаемого типа аргумента, возвращаемого значения или свойства: function save(User $user): void. PHP при вызове проверит, что передан именно объект User (или его наследник/реализация интерфейса) — иначе бросит TypeError. Делает контракты явными, помогает IDE и статическому анализу, ловит ошибки раньше — до запуска. Поддерживаются скаляры (int, string, float, bool), array, объекты по имени класса/интерфейса, nullable (?Type), union (int|string), self/static/void/never.',
                'code_example' => '<?php
