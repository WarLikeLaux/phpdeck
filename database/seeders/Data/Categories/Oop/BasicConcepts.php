...
echo $user->greet(); // Привет, я Иван',
            ...
            [
                ...
                'answer' => 'Парадигма — это **стиль мышления о коде**. Делятся на две большие ветки:

**1. Императивная** — описываем шаг за шагом, КАК выполнить задачу:

- **Процедурная** — программа = набор процедур и глобальных данных (C, ранний PHP).
- **ООП** — программа = набор взаимодействующих объектов (PHP, Java, C#).

**2. Декларативная** — описываем, ЧТО хотим получить, а не как:

- **Функциональная** — композиция чистых функций без изменяемого состояния (Haskell, Elixir).
- **Логическая** — правила и факты, движок сам ищет решение (Prolog).
- **SQL** — `SELECT ...` описывает результат, оптимизатор решает план.

**PHP — мультипарадигменный:** одновременно поддерживает процедурный, ООП и функциональный стили (`array_map`, замыкания, first-class callable).',
                'code_example' => null,
                'code_language' => null,
            ],
            [
...
echo $u->greet(); // Hi, Иван',
            ...
            [
                ...
                'answer' => '`self::` ссылается на **класс**, `$this` — на **объект**.

- `self::` — доступ к **статическим** членам и **константам** того класса, где написан код: `self::COUNT`, `self::$instances`, `self::method()`.
- `$this->` — доступ к **обычным** свойствам и методам **текущего экземпляра**.

**Особенности:**

- В статическом методе `$this` **недоступен** — объекта-то нет.
- `self::` всегда указывает на класс, где **физически написан** код, даже если метод унаследован. Если нужно «класс, через который реально вызвали» — это `static::` (Late Static Binding).',
                'difficulty' => 2,
                'topic' => 'oop.basic_concepts',
                'code_example' => '<?php
class Counter
    ...
    public static function reset(): void
    {
        self::$count = 0; // здесь $this недоступен
            ...
            [
                ...
                'answer' => '**Нет.** `new AbstractClass()` бросит `Error: Cannot instantiate abstract class`.

**Почему:** в абстрактном классе могут быть **методы без тела** (`abstract public function area()`). Создать такой объект — значит получить объект с «дырой» в методе, что небезопасно.

**Что делать:**

1. Наследовать — `class Circle extends Shape`.
2. Реализовать все `abstract`-методы.
3. Создавать экземпляр **наследника** — `new Circle(2.0)`.

Сам абстрактный класс можно использовать как **тип** в typehint: `function draw(Shape $s)` примет любого наследника.',
                'difficulty' => 2,
                'topic' => 'oop.basic_concepts',
                'code_example' => '<?php
    ...
    public function check(int $count): bool
    {
        return $count <= self::MAX_ITEMS;
            ...
            [
                ...
                'answer' => '`implements` — объявление, что класс **реализует контракт** интерфейса.

- `class Email implements Sendable` — `Email` **обязан** содержать все методы из `Sendable` с совместимыми сигнатурами.
- Можно реализовать **сразу несколько** интерфейсов: `class Foo implements Sendable, Cacheable, Loggable`.
- В отличие от `extends` (только один родитель), `implements` не имеет ограничения по количеству.
- Любой такой класс можно подставить туда, где ждут интерфейс: `function deliver(Sendable $s)` примет `Email`, `Sms`, `Push`.

**Главное:** `implements` — это **обещание API**, без реализации поведения (в отличие от `extends` или `use trait`).',
                'difficulty' => 2,
                'topic' => 'oop.basic_concepts',
                'code_example' => '<?php
...
$a->ban();        // свой метод',
            ...
            [
                ...
                'answer' => '**Override** — наследник заменяет реализацию родительского метода своей, сохраняя совместимую сигнатуру.

**Как работает:**

- Вызов `$child->speak()` идёт в **версию наследника** — это и есть полиморфизм.
- К старой реализации можно обратиться через `parent::speak()` (часто полезно: «сначала родитель, потом своё»).
- Сигнатура должна быть **совместима** (LSP): параметры — contravariant, возврат — covariant.

**Атрибут `#[\\Override]` (PHP 8.3):** не обязателен, но если поставить — PHP проверит, что метод **действительно** существует у родителя. Защита от опечаток (`spaek` вместо `speak` — IDE/runtime поймает сразу, а не молча создаст новый метод).',
                'code_example' => '<?php
class Animal
{
