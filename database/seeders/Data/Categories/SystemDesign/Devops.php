    ...
    public static function all(): array
    {
        return [
            [
                ...
                'answer' => 'Деплой (deploy, развёртывание) — это процесс выкатки новой версии приложения на сервер, чтобы её увидели пользователи. Аналогия: ты доделал фичу локально на своём ноуте — теперь надо положить её на боевой сервер так, чтобы сайт продолжал работать. Минимальный деплой Laravel-приложения: 1) скачать новый код (git pull), 2) поставить зависимости (composer install --no-dev), 3) накатить миграции БД (php artisan migrate), 4) очистить и прогреть кеши (php artisan config:cache, route:cache), 5) перезапустить процессы (php-fpm, queue:restart). На современном проекте это автоматизировано через CI/CD: пушнул в main → пайплайн сам собрал, прогнал тесты и раскатал. Бывают разные стратегии: rolling (по одной машине), blue-green (две среды, переключение), canary (выкатка на часть пользователей).',
...
sudo systemctl reload php8.3-fpm',
            ...
            [
                ...
                'answer' => 'Это разделение приложения на изолированные среды с одинаковым кодом, но разными данными, ключами и настройками. dev (development) — локальная машина разработчика или общий dev-сервер, можно ломать что угодно. stage (staging) — максимально похожая на прод копия, на ней QA и разработчики проверяют релизы перед выкаткой: миграции, интеграции с внешними API, нагрузку. prod (production) — боевое окружение, которым пользуются клиенты, тут реальные деньги и реальные данные. Зачем разделение: ловить ошибки до пользователей, не сломать боевую БД при экспериментах, проверять миграции на безопасной копии, иметь возможность отлаживать без риска. В Laravel переключение через переменную окружения APP_ENV в файле .env — она доступна как app()->environment() и определяет, какие конфиги, ключи, базы используются.',
