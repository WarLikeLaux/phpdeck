    ...
    public static function all(): array
    {
        return [
            [
                ...
                'answer' => 'Деплой (deploy, развёртывание) — выкатить новую версию приложения на сервер так, чтобы её увидели пользователи. Простыми словами: ты доделал фичу локально, теперь надо положить её на боевой сервер. Минимальный деплой: скачать код, поставить зависимости (composer install), накатить миграции БД, очистить кеши, перезапустить процессы. На современном проде это автоматизировано через CI/CD: пуш в main → пайплайн собирает → раскатывает.',
                'difficulty' => 1,
            ...
            [
                ...
                'answer' => 'Разделение приложения на изолированные среды с одинаковым кодом, но разными данными и настройками. dev (development) — локальная машина разработчика. stage (staging) — копия прод-окружения для тестирования релизов перед выкаткой. prod (production) — боевое окружение, которым пользуются клиенты. Зачем: ловить ошибки до пользователей, не трогать боевую БД, проверять миграции и интеграции на безопасной копии. Конфиг переключается через переменные окружения (APP_ENV в Laravel).',
                'difficulty' => 1,
                'topic' => 'system_design.devops',
            ],
            [
