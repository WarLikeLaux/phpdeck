    ...
    public static function all(): array
    {
        return [
            [
                ...
                'answer' => 'HTTPS = HTTP + TLS. TLS (Transport Layer Security) — криптографический протокол, шифрующий передачу данных по сети. Простыми словами: HTTP — открытка, которую может прочесть любой почтальон, HTTPS — запечатанный конверт. Решает три задачи: 1) шифрование (никто не подслушает по дороге), 2) аутентификация сервера через сертификат (это действительно тот сайт, а не подделка), 3) целостность (данные не подменены в пути). SSL — устаревший предшественник TLS; в проде сегодня держат только TLS 1.2 и 1.3.',
                'difficulty' => 2,
