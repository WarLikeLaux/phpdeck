    ...
    public static function all(): array
    {
        return [
            [
                ...
                'answer' => 'Архитектура — это крупные решения о том, как устроено приложение: на какие слои и модули оно разбито, кто с кем общается, где лежат данные, как масштабируется. Аналогия: чертёж дома — где несущие стены, где окна, где лестница. Код можно переписать за день, архитектуру — за полгода, поэтому решения принимают осознанно и заранее. На уровне junior важно знать пару базовых стилей: монолит (всё в одном приложении), микросервисы (много мелких сервисов), MVC (разделение на Model-View-Controller внутри одного приложения). И помнить: нет «правильной» архитектуры — есть подходящая под задачу, команду и нагрузку. Для пет-проекта микросервисы — оверкилл, для Netflix монолит — узкое место.',
                'difficulty' => 1,
...
# Так legacy не загрязняет новый код своей семантикой.',
            ...
            [
                ...
                'answer' => 'Model-View-Controller — паттерн, который делит приложение на 3 слоя с чёткими ролями. Model — данные и бизнес-логика: что такое User, Order, как они сохраняются в БД, как считаются. View — представление, то что видит пользователь: HTML-страница, JSON-ответ для API. Controller — связующее звено: принимает HTTP-запрос, дёргает нужную Model, передаёт результат во View. Идея — каждый слой отвечает за своё, можно менять UI не трогая логику и наоборот, и тестировать слои отдельно. Большинство веб-фреймворков (Laravel, Symfony, Rails, ASP.NET MVC) построены вокруг этого паттерна. В Laravel: маршрут → метод контроллера в app/Http/Controllers → модель Eloquent в app/Models → шаблон Blade или JSON-ресурс.',
