    ...
    location / {
        ...
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
            ...
            [
                ...
                'answer' => 'Load balancer - распределяет входящие запросы между несколькими серверами, скрывая их за единым адресом. Решает три задачи: масштабирование (можно добавлять/убирать backend-узлы), отказоустойчивость (через health checks выводит из ротации упавшие узлы), удобный entrypoint для TLS termination и rate limiting. Sticky session — привязка пользователя к одному и тому же серверу для всех его запросов; нужна, когда сессии хранятся в памяти конкретного сервера и не вынесены в общее хранилище (Redis/Memcached). Конкретные алгоритмы балансировки (round robin, least connections, IP hash, power of two, L4 vs L7) — см. отдельную карточку в performance.',
