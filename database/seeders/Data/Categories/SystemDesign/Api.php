...
DELETE /api/users/42    - удалить',
            ...
            [
                ...
                'answer' => 'REST — HTTP + JSON, ресурсо-ориентированный (URL = существительное, действие = метод), клиент берёт всё что отдаёт endpoint, штатно кэшируется через HTTP-кэш по URL — стандарт для публичных API и CRUD. GraphQL — один POST-endpoint /graphql, клиент в запросе сам указывает какие поля нужны (нет over/under-fetching), сильная типизация через SDL; цена — стандартный HTTP-кэш по умолчанию не работает (POST с JSON-телом), нужен Persisted Queries / DataLoader / query complexity analysis. gRPC — HTTP/2 + Protocol Buffers, бинарный, контракт через .proto и кодогенерация на 11+ языков, поддерживает 4 типа RPC (unary + server/client/bidirectional streaming), быстрый и компактный. Выбор по сценарию: REST — публичные API и CRUD, GraphQL — богатый UI с разными view (BFF), gRPC — межсервисное общение с низкой задержкой и стримингом. Для браузеров gRPC нужен grpc-web (overhead), для CDN-кэширования — REST.',
                'difficulty' => 3,
                'topic' => 'system_design.api',
            ],
            [
...
// или cookie-based: sticky cookie srv_id expires=1h domain=.example.com path=/;',
            ...
            [
                ...
                'answer' => 'Offset (LIMIT N OFFSET M) — просто и поддерживает прыжки на N-ю страницу, но при больших M база сканирует и отбрасывает M строк (тормозит на 100k+), и при вставке новых записей возможны дубли/пропуски между страницами. Cursor (keyset) — WHERE id > :last_id ORDER BY id LIMIT N: сложность O(log N + limit) по индексу (seek по B-tree до позиции курсора + чтение limit строк), не зависит от номера страницы и стабильна при вставках. Минус cursor — нельзя прыгнуть на N-ю страницу, только next/prev. Важно при сортировке по неуникальному полю: если ORDER BY created_at, а курсор хранит только last_created_at, при одинаковых created_at соседних строк получаются пропуски (по >) или дубли (по >=). Решение — составной курсор с tiebreaker по уникальному id: WHERE (created_at, id) > (:last_at, :last_id) ORDER BY created_at, id LIMIT N. Правило: offset для админки с малыми объёмами и нумерацией страниц, cursor — для бесконечных лент, публичных API и больших таблиц.',
