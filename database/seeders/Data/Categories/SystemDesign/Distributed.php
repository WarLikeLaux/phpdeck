    ...
    public static function all(): array
    {
        return [
            [
                ...
                'answer' => 'Группа компьютеров, которые работают над общей задачей и общаются по сети, но для пользователя выглядят как одна программа. Простыми словами: вместо одного большого сервера у тебя десять маленьких, и пользователь не знает, на каком из них обработался его запрос. Зачем: масштабирование (больше пользователей), отказоустойчивость (упал один — остальные продолжат), географическая близость (сервер в Москве и в Сингапуре). Сложности: сеть ненадёжна, узлы падают, часы расходятся, нужно договариваться о порядке событий — отсюда вся теория про CAP, consensus и saga.',
                'difficulty' => 2,
