...
usleep($delay * 1000);',
            ...
            [
                ...
                'answer' => 'Master принимает все записи и асинхронно (или полусинхронно) проигрывает лог изменений (binlog в MySQL, WAL в Postgres, oplog в MongoDB) на одну или несколько реплик-slave. Современная терминология — primary/replica или leader/follower; master/slave признана устаревшей. Это даёт три выигрыша: горячий резерв на случай падения мастера (failover), отделённое чтение для аналитики и тяжёлых отчётов, чтобы не мешать OLTP, и геораспределённые реплики ближе к клиенту. Главный компромисс — реплика всегда чуть отстаёт, и при чтении сразу после записи можно увидеть старую версию данных. Репликация не заменяет бэкапы: ошибочный DELETE мгновенно прилетит на все реплики.',
                'difficulty' => 3,
                'topic' => 'system_design.distributed',
            ],
            [
