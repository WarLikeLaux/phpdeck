    ...
    public static function all(): array
    {
        return [
            [
                ...
                'answer' => 'Автотест — кусок кода, который сам проверяет другой кусок кода. Зачем: 1) Уверенность при изменениях — поменял что-то, прогнал тесты, увидел что не сломал соседнее. 2) Документация поведения — читаешь имя теста («user_can_login_with_valid_credentials») и сразу видишь, как код должен работать. 3) Быстрая обратная связь — баг ловится в CI за секунды, а не на проде через неделю от пользователя. 4) Защита от регрессий — старая фича не ломается, когда добавляют новую. 5) Экономия времени — ручная проверка десятка сценариев занимает час, тесты — несколько секунд. Без тестов рефакторинг превращается в лотерею: «вроде работает, а ломаем ли мы что-то — узнаем от пользователей».',
            ...
            [
                ...
                'answer' => 'Тест на одну маленькую единицу кода — метод, функция, класс — В ИЗОЛЯЦИИ от всего остального. БД, сеть, файлы, время не используются по-настоящему: либо они не нужны для этой логики, либо подменяются mock-ом/stub-ом. Признаки хорошего unit-теста: 1) Быстрый — миллисекунды, тысячи штук прогоняются за пару секунд. 2) Детерминированный — на одном коде всегда даёт один результат. 3) Локальный диагноз — если упал, понятно, что баг именно в этой функции, а не «где-то в цепочке». Типичный кейс — чистая логика без зависимостей: калькулятор, валидатор, форматтер цены, парсер строки. Если в тесте есть RefreshDatabase или $this->get(\'/route\') — это уже не unit, а feature/integration.',
            ...
            [
                ...
                'answer' => 'Тест, проверяющий, что несколько компонентов работают ВМЕСТЕ как ожидается. Типичный набор: сервис + Eloquent-модель + реальная (но тестовая) БД, или контроллер + кеш + Redis. Медленнее unit-теста (секунды, а не миллисекунды), потому что поднимается БД/инфраструктура. Зато ловит то, что в изоляции не видно: опечатка в SQL, неверная миграция, плохой JOIN, неправильная связь в relation, забытый индекс. Главное отличие от unit: integration-тест трогает «настоящие» границы (БД, файловая система, очередь), но обычно без HTTP-слоя сверху — это уже зона feature-теста.',
            ...
            [
                ...
                'answer' => 'End-to-end («от края до края») — тест, проверяющий приложение целиком как живой пользователь: запускает настоящий браузер, открывает URL, кликает кнопки, заполняет формы, ждёт ответ, читает текст на странице. Самый медленный тип (секунды-минуты на тест) и самый хрупкий (упал — может быть и баг, и просто сеть моргнула), но и самый «реалистичный»: проверяет front + back + БД + JS вместе. Инструменты: Cypress, Playwright, Selenium, в Laravel — Dusk (запускает Chrome через ChromeDriver). По пирамиде тестирования таких тестов держат немного — только критические user-flow: логин, оплата, регистрация. Всё остальное проверяют дешёвыми unit/feature-тестами.',
            ...
            [
                ...
                'answer' => 'Утверждение «должно быть так-то», которое тест проверяет. Если так — assert молча проходит дальше. Если не так — тест падает с понятным сообщением вида «Failed asserting that 4 is identical to 5». Без assert-ов тест ничего не проверяет: просто прогоняет код и всегда зелёный — это распространённая ошибка джунов. Базовые PHPUnit-ассерты: assertSame (строгое ===), assertEquals (нестрогое ==), assertTrue/assertFalse, assertNull, assertCount, assertInstanceOf, assertContains. В Laravel сверху — response-ассерты: $response->assertStatus(200), assertOk(), assertJson([...]), assertJsonFragment([...]), assertRedirect(\'/url\'), assertSee(\'текст\'), а также database-ассерты: assertDatabaseHas, assertDatabaseCount.',
