...
class Basics
{
    public static function all(): array
    {
        return [
            ...
            [
                'category' => 'Тестирование',
                'question' => 'Чем unit, integration и feature тесты отличаются на практике?',
                'answer' => "Один и тот же код можно покрыть тестами трёх «глубин», и важно не путаться, **что** ты сейчас проверяешь.\n\n| Признак | Unit | Integration | Feature (Laravel) |\n| --- | --- | --- | --- |\n| Что проверяет | один класс/метод | склейку нескольких компонентов | HTTP-эндпоинт от запроса до ответа |\n| БД | **нет** (либо in-memory) | **да**, тестовая | **да**, тестовая |\n| HTTP-слой | нет | обычно нет | **да** |\n| Скорость | мс | секунды | секунды |\n| Что ловит | логические баги | стыковочные (`JOIN`, ORM, миграции) | целиком user-flow в API |\n| Сколько штук | **много** (низ пирамиды) | средне | средне-много |\n\n**Решающий вопрос «какой тип писать»:**\n\n- Чистая бизнес-логика без зависимостей → **unit**.\n- Сервис + Eloquent / репозиторий + БД, без HTTP → **integration**.\n- `POST /api/users` → ответ + запись в БД + уведомление → **feature**.\n\n**Антипаттерн**: называть «unit-тестом» что-то с `RefreshDatabase` или `\$this->get('/url')` — это уже integration/feature. Назначение папки `tests/Unit/` — изолированные классы, **без** Laravel-бутстрапа.",
                'code_example' => "<?php\n\n// 1) UNIT — чистая логика, без БД/HTTP\nfinal class DiscountCalculatorTest extends \\PHPUnit\\Framework\\TestCase\n{\n    public function test_applies_percentage(): void\n    {\n        \$this->assertSame(80.0, (new DiscountCalculator())->apply(100.0, 0.2));\n    }\n}\n\n// 2) INTEGRATION — сервис + БД, без HTTP\nclass UserRegistrationServiceTest extends \\Tests\\TestCase\n{\n    use \\Illuminate\\Foundation\\Testing\\RefreshDatabase;\n\n    public function test_creates_user_record(): void\n    {\n        app(UserRegistrationService::class)->register('jane@example.com', 'secret123');\n        \$this->assertDatabaseHas('users', ['email' => 'jane@example.com']);\n    }\n}\n\n// 3) FEATURE — HTTP-эндпоинт целиком\nclass RegisterEndpointTest extends \\Tests\\TestCase\n{\n    use \\Illuminate\\Foundation\\Testing\\RefreshDatabase;\n\n    public function test_register_endpoint_creates_user_and_redirects(): void\n    {\n        \$response = \$this->post('/register', [\n            'email' => 'jane@example.com',\n            'password' => 'secret123',\n            'password_confirmation' => 'secret123',\n        ]);\n\n        \$response->assertRedirect('/dashboard');\n        \$this->assertDatabaseHas('users', ['email' => 'jane@example.com']);\n    }\n}",
                'code_language' => 'php',
                'difficulty' => 2,
                'topic' => 'testing.basics',
            ],
            [
                'category' => 'Тестирование',
                'question' => 'Что такое contract test и зачем он между сервисами?',
                'answer' => "**Contract test** — тест, который проверяет, что **API между двумя сервисами** соответствует общему «контракту» (схеме): какие поля, типы, статус-коды, заголовки.\n\n**Проблема, которую решает**: микросервис `A` обновил `/users` и убрал поле `phone`. Сервис `B` его читает — **сломался на проде**. Юнит-тесты обоих сервисов **зелёные**, e2e гонять долго и дорого.\n\n**Идея**:\n\n- **Consumer** (`B`) пишет, **какой ответ** он ожидает от `/users` — это и есть контракт.\n- **Provider** (`A`) при сборке прогоняет тест: **отдаёт ли мой эндпоинт ровно то, что ждёт `B`**.\n- Контракт хранится в общем брокере (`Pact Broker`) или в git-репо.\n\n**Когда применять**:\n\n- Микросервисы / связка backend ↔ мобильное приложение.\n- Связка backend ↔ SPA с фиксированным API-клиентом.\n- НЕ применять для одного монолита — там обычные feature-тесты дешевле.\n\n**Инструменты**:\n\n- **`Pact`** — стандарт де-факто, есть PHP-клиент (`pact-php`).\n- **OpenAPI / JSON Schema** — лёгкий вариант: храним схему, в CI валидируем ответы тестов через `justinrainbow/json-schema`.",
                'code_example' => "<?php\n\n// Consumer-side (сервис B) — пишем ожидание контракта на PHP через pact-php\nuse PhpPact\\Consumer\\InteractionBuilder;\nuse PhpPact\\Consumer\\Model\\ConsumerRequest;\nuse PhpPact\\Consumer\\Model\\ProviderResponse;\n\npublic function test_user_api_contract(): void\n{\n    \$builder = new InteractionBuilder(\$this->pactConfig);\n\n    \$builder\n        ->given('User 42 exists')\n        ->uponReceiving('a request for user 42')\n        ->with((new ConsumerRequest())->setMethod('GET')->setPath('/users/42'))\n        ->willRespondWith(\n            (new ProviderResponse())\n                ->setStatus(200)\n                ->addHeader('Content-Type', 'application/json')\n                ->setBody(['id' => 42, 'email' => 'jane@example.com'])\n        );\n\n    \$user = (new UserApiClient(\$this->pactConfig->getMockServer()))->find(42);\n\n    \$this->assertSame('jane@example.com', \$user->email);\n    \$builder->verify(); // публикует pact-файл, его потом скачивает provider в своём CI\n}",
                'code_language' => 'php',
                'difficulty' => 3,
                'topic' => 'testing.basics',
            ],
            [
                'category' => 'Тестирование',
                'question' => 'Что такое mutation testing простыми словами?',
                'answer' => "**Mutation testing** — способ проверить **качество самих тестов**, а не код. Инструмент берёт код и **намеренно ломает** его по чуть-чуть: `>` → `>=`, `+` → `-`, `true` → `false`, `&&` → `||`. Каждое такое изменение — **мутант**.\n\nДальше прогоняет твои тесты:\n\n- **Мутант убит** — хотя бы один тест упал, поймал поломку. **Хорошо**.\n- **Мутант выжил** — все тесты остались зелёными после поломки кода. **Плохо** — значит, либо нет теста на эту ветку, либо тест слишком слабый (нет нужного assert-а).\n\n**MSI (Mutation Score Indicator)** = `убитые / все мутанты`. Норма для боевого кода — **70-85%**.\n\n**Зачем нужно**, если есть code coverage:\n\n- **Coverage** показывает, **прошёл ли** код через тест. 100% coverage возможны при тестах без assert-ов вообще.\n- **MSI** показывает, **проверяет ли** тест поведение. Coverage 100% + MSI 30% = тесты бесполезны.\n\n**Минусы**: медленно (запуск всех тестов для каждой мутации). Запускают **ночью в CI**, не на каждый коммит.",
                'code_example' => "<?php\n\n// Исходный код\nclass DiscountCalculator\n{\n    public function apply(float \$price, float \$rate): float\n    {\n        if (\$rate > 0) {                    // ← мутация: > → >=\n            return \$price * (1 - \$rate);    // ← мутация: * → /\n        }\n        return \$price;\n    }\n}\n\n// Слабый тест — coverage 100%, но мутанта НЕ убьёт\npublic function test_returns_a_number(): void\n{\n    \$this->assertIsFloat((new DiscountCalculator())->apply(100, 0.2));\n}\n\n// Хороший тест — убивает мутанта *→/\npublic function test_twenty_percent_off_a_hundred_is_eighty(): void\n{\n    \$this->assertSame(80.0, (new DiscountCalculator())->apply(100, 0.2));\n}\n\n// Хороший тест — убивает мутанта > → >=\npublic function test_zero_rate_returns_original(): void\n{\n    \$this->assertSame(100.0, (new DiscountCalculator())->apply(100, 0.0));\n}",
                'code_language' => 'php',
                'difficulty' => 3,
                'topic' => 'testing.basics',
            ],
