    ...
    public static function all(): array
    {
        return [
            ...
            [
                ...
                'answer' => "**Data provider** — способ запустить **один и тот же тест с разными входными данными** без копипасты. Метод-провайдер отдаёт **наборы аргументов**, `PHPUnit` прогоняет тест по каждому набору как **отдельный кейс**.\n\nТребования к методу-провайдеру:\n\n- **`public static`** (обязательно с `PHPUnit 10+`, иначе `RiskyTest`).\n- Возвращает `array` **или** `Generator` (`yield` — удобно при большом числе кейсов, не грузит всё в память сразу).\n- Имя теста должно знать про провайдер: атрибут **`#[DataProvider('methodName')]`** в `PHPUnit 10+` или аннотация **`@dataProvider methodName`** в докблоке (`PHPUnit 9`).\n\nИмена кейсов:\n\n- **Ключи** массива становятся именами в выводе: `test_rejects_invalid_email with data set \"empty\"`.\n- Поэтому именуй ключи **смыслово** (`'no @'`, `'spaces'`), а не `0/1/2` — при падении сразу видно, какой кейс упал.\n\nТипичные сценарии:\n\n- **Граничные случаи** — пустая строка, очень длинная, спецсимволы, `null`.\n- **Таблицы валидации** — много правил, один и тот же `assert`.\n- **Матрица «вход → ожидаемый выход»** — `[вход, ожидание]`, ожидание прилетает вторым аргументом теста.\n\nГотчи:\n\n- Провайдер вызывается **до `setUp()`** — нельзя использовать `\$this` или зависящие от контейнера Laravel-вещи.\n- `Pest`-аналог — **`->with([...])`** или именованный `dataset('name', [...])`.",
                'code_example' => "<?php\n\nuse PHPUnit\\Framework\\Attributes\\DataProvider;\nuse PHPUnit\\Framework\\TestCase;\n\nclass EmailValidatorTest extends TestCase\n{\n    // Простой кейс: один аргумент → assertFalse\n    #[DataProvider('invalidEmails')]\n    public function test_rejects_invalid_email(string \$email): void\n    {\n        \$this->assertFalse(EmailValidator::isValid(\$email));\n    }\n\n    // Матрица «вход → ожидание»: оба значения прилетают аргументами\n    #[DataProvider('discountCases')]\n    public function test_discount(float \$total, float \$expected): void\n    {\n        \$this->assertSame(\$expected, (new DiscountCalculator())->apply(\$total));\n    }\n\n    // public static — обязательное требование PHPUnit 10+\n    public static function invalidEmails(): array\n    {\n        return [\n            // именованные ключи попадут в имя кейса: \"with data set 'no @'\"\n            'empty'     => [''],\n            'no @'      => ['plainstring'],\n            'no domain' => ['user@'],\n            'spaces'    => ['a b@example.com'],\n        ];\n    }\n\n    // Generator — экономит память на больших наборах\n    public static function discountCases(): \\Generator\n    {\n        yield 'below threshold' => [50.0, 50.0];\n        yield 'exact threshold' => [100.0, 90.0];\n        yield 'over threshold'  => [250.0, 200.0];\n    }\n}",
                'code_language' => 'php',
                'difficulty' => 3,
                'topic' => 'testing.practice',
            ],
            ...
            [
                ...
                'answer' => "Главное правило — **не звать `new DateTime()` / `time()` / `date()` напрямую в коде**, а ходить через **`Carbon::now()`** или хелпер **`now()`** в Laravel. Только тогда время можно «заморозить» в тесте — иначе зависимость от системного времени останется, и тест станет flaky на границе суток / месяца / часового пояса.\n\nДва уровня инструментов:\n\n- **Голый `PHPUnit` + `Carbon`** — `Carbon::setTestNow('2025-01-01 12:00:00')`. Все вызовы `now()` будут возвращать эту дату. **Обязательно** в `tearDown()` зови `Carbon::setTestNow()` **без аргументов** — иначе следующие тесты увидят замороженное время.\n- **Laravel-хелперы** — `\$this->travelTo(\$date)`, `\$this->travel(31)->days()`, `\$this->freezeTime()`. Фреймворк сам сбрасывает время после теста, забыть **невозможно**.\n\nКлассические сценарии:\n\n- **Истечение** подписки / токена / промокода через `N` дней.\n- **Cron-логика** — что произойдёт в полночь / в начале месяца.\n- **Кеш с `TTL`** — что выкинется через `60s`.\n- **Аудит-таймстампы** — фиксированный `created_at` для сравнения снапшотов.\n\nАльтернатива на уровне **дизайна** — внедрять **`Clock`-интерфейс** (`PSR-20`, стандарт): прод-реализация `SystemClock` возвращает `new DateTimeImmutable()`, в тестах подменяешь на `FixedClock` через DI. Чище, чем глобальный `setTestNow`, но требует прокидывать `Clock` всюду, где код смотрит на время.",
