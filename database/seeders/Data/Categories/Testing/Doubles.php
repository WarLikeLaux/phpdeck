    ...
    public static function all(): array
    {
        return [
            ...
            [
                ...
                'answer' => "**Spy** — подменка, которая **молча записывает** все вызовы (какие методы, с какими аргументами, сколько раз) и позволяет проверить это **после** действия, отдельными `assert`-ами.\n\nОтличие от `mock`:\n\n- **Mock** — ожидания **до** действия, падает сам в `tearDown()`.\n- **Spy** — никаких ожиданий заранее, проверки делаются явно **после**.\n\nЗа что любят:\n\n- Чистый **AAA-порядок** (`Arrange-Act-Assert`) — настройка → действие → проверка идут по порядку, без «забегания вперёд» с ожиданиями.\n- Тест **не привязан** к деталям интеракции на старте — можно проверить только то, что реально важно.\n\nГлавный минус:\n\n- `assert` можно **случайно забыть** — без него тест остаётся **зелёным** при отсутствии важного вызова. Mock тут безопаснее: ожидание срабатывает само.\n\nВ Laravel **`Mail::fake`**, **`Bus::fake`**, **`Queue::fake`**, **`Event::fake`**, **`Notification::fake`** работают именно как spy — записывают вызовы, чтобы потом `assertSent` / `assertDispatched` / `assertSentTo` проверили факт.",
            ...
            [
                ...
                'answer' => "Главное различие — **что именно проверяет тест**:\n\n| Свойство | **Stub** | **Mock** |\n|---|---|---|\n| Про что | про **данные** | про **взаимодействие** |\n| Подход | **state-based** (итоговое состояние) | **interaction-based** (факт вызова) |\n| Что задаёт | «когда вызовут — верни вот это» | «жду вызов 1 раз с такими аргументами» |\n| Кто падает | падает обычный `assert*` теста | падает сам объект в `tearDown()` |\n| API в `PHPUnit` | `createStub()` + `->method()->willReturn()` | `createMock()` + `->expects(\$this->once())` |\n\n**Правило джуна**:\n\n- Если падающий `assert` проверяет «**что вернулось / лежит в БД**» — это **stub**.\n- Если проверяет «**был ли вызов**» — это **mock**.\n\nНа практике один объект **может играть обе роли** (`PHPUnit` / `Mockery` это позволяют в одном `createMock()`), но **смешивать дорого**: тест становится одновременно «про данные и про звонки», растёт сцепление с реализацией — любая правка кода рушит тест по двум причинам сразу.",
            ...
            [
                ...
                'answer' => "Оба проверяют **факт вызова**, но в разные моменты:\n\n| Свойство | **Mock** | **Spy** |\n|---|---|---|\n| Когда задаются ожидания | **до** действия (`Arrange`) | **не задаются заранее** |\n| Где `assert` | внутри ожидания (`expects`) | **после** действия, отдельной строкой |\n| Кто падает | сам объект в `tearDown()` | обычный `assert*` теста |\n| Порядок чтения | «забегает вперёд» — ожидание видно до действия | чистый **AAA**: `Arrange → Act → Assert` |\n| Риск «зелёного теста без проверки» | низкий — ожидание сработает само | **высокий** — `assert` можно забыть |\n\nВ `PHPUnit` чистого spy «из коробки» нет — обычно либо `createMock()` без `expects()` (а проверка отдельным методом), либо `Mockery::spy(...)` + `->shouldHaveReceived('m')`.\n\nВ Laravel spy-стиль **дефолтный**: **`Bus::fake`**, **`Mail::fake`**, **`Queue::fake`**, **`Event::fake`**, **`Notification::fake`** записывают вызовы, а после действия зовутся `assertDispatched` / `assertSent` / `assertSentTo`. Главный плюс — естественное чтение по AAA; главный минус — без явного `assert` тест зелёный, даже если ничего не отправилось.",
