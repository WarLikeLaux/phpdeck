    ...
    public static function all(): array
    {
        return [
            ...
            [
                ...
                'answer' => 'Подменяет HTTP-клиент Laravel (Http::get/post) фейком, чтобы не дёргать реальные внешние API в тестах. Можно задать заранее заготовленные ответы по URL-маске и потом проверить, что нужный запрос был сделан. Аналоги для очередей — Bus::fake(), для событий — Event::fake(), для писем — Mail::fake().',
                'code_example' => "<?php\n\nuse Illuminate\\Support\\Facades\\Http;\n\npublic function test_fetches_user_from_external_api(): void\n{\n    Http::fake([\n        'api.example.com/users/42' => Http::response([\n            'id' => 42,\n            'name' => 'Jane',\n        ], 200),\n    ]);\n\n    \$user = (new ExternalUserClient())->find(42);\n\n    \$this->assertSame('Jane', \$user->name);\n    Http::assertSent(fn (\$req) => \$req->url() === 'https://api.example.com/users/42');\n}",
                'code_language' => 'php',
                'difficulty' => 3,
                'topic' => 'testing.tools',
            ],
