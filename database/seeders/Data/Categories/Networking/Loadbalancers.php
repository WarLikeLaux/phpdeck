    ...
    public static function all(): array
    {
        return [
            ...
            [
                ...
                'answer' => 'Сервер-распределитель, который стоит перед группой бэкендов: принимает входящие запросы и раскидывает их между несколькими серверами. Как администратор в очереди — направляет посетителей к свободной кассе. Зачем нужен: 1) Масштабирование — один сервер не справляется, поставили десять, балансировщик размазывает нагрузку. 2) Отказоустойчивость — если один сервер упал, балансировщик через health-check перестаёт слать ему трафик и распределяет остальной по живым. 3) Релиз без даунтайма — выводят серверы по очереди, обновляют, возвращают. Примеры: nginx, HAProxy, AWS ALB/NLB, Cloudflare Load Balancer, Kubernetes Service.',
                'difficulty' => 1,
                'topic' => 'networking.loadbalancers',
            ],
            [
                ...
                'answer' => 'Правило балансировщика: «все запросы одного пользователя — на ОДИН и тот же бэкенд». Реализуется через cookie (балансировщик ставит свою или читает приложение-сессионную) или хеш src_ip. Нужно, когда состояние клиента хранится локально на сервере: in-memory сессии, WebSocket/SSE-соединение, разогретый кэш. Минусы: при падении сервера его пользователи теряют сессию, нагрузка распределяется неравномерно (hot spots), сложнее масштабироваться. Современная практика — НЕ полагаться на sticky: класть сессию в общий Redis/БД, тогда любой запрос можно отдать любому бэкенду.',
                'difficulty' => 2,
                'topic' => 'networking.loadbalancers',
            ],
            [
