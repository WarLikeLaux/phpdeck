    ...
    public static function all(): array
    {
        return [
            ...
            [
                ...
                'answer' => 'Сервер-распределитель, который стоит перед группой бэкендов: принимает входящие запросы и раскидывает их между несколькими серверами. Как администратор в очереди — направляет к свободной кассе. Зачем: 1) Масштабирование — один сервер не справляется, поставили десять. 2) Отказоустойчивость — если один сервер упал, балансировщик через health-check перестаёт слать ему трафик и распределяет остальной по живым. Примеры: nginx, HAProxy, AWS ALB, Cloudflare Load Balancer.',
                'difficulty' => 1,
                'topic' => 'networking.loadbalancers',
            ],
            [
