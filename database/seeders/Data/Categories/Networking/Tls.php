...
class Tls
{
    public static function all(): array
    {
        return [
            ...
            [
                'category' => 'Сети',
                'question' => 'Как проверить сертификат сайта руками?',
                'answer' => 'Через openssl s_client можно увидеть весь сертификат и цепочку до CA. Полезно при отладке ошибок типа «certificate verify failed». В браузере можно посмотреть через значок замка → детали сертификата. Онлайн-проверка: ssllabs.com/ssltest даёт развёрнутый отчёт о настройках TLS.',
                'code_example' => "openssl s_client -connect example.com:443 -servername example.com\n# Показывает цепочку сертификатов\nopenssl s_client -connect example.com:443 -showcerts < /dev/null 2>/dev/null | openssl x509 -noout -dates\n# notBefore=... notAfter=... — сроки",
                'code_language' => 'bash',
                'difficulty' => 3,
                'topic' => 'networking.tls',
            ],
