...
Route::get(\'/profile/{id}\', ShowProfile::class);',
            ...
            [
                ...
                'answer' => 'Laravel **автоматически** разрешает зависимости в **конструкторе** и **методах действий** контроллеров через **Service Container**. Достаточно указать **type-hint** — контейнер сам подставит нужный объект.

**Места, где работает auto-DI:**
- **`__construct`** — общие зависимости (репозитории, сервисы), создаются на каждый запрос **один раз**.
- **Методы-экшены** (`store`, `update`...) — зависимости только для этого действия (минимизирует резолв того, что не нужно).
- **FormRequest** — `StoreUserRequest $request` автоматически инстанциируется, валидируется до входа в метод.
- **Route Model Binding** — `Post $post` в методе подставляет модель по `id` из URL.

**Под капотом:**
1. Laravel **рефлексией** читает сигнатуру (PHP `ReflectionMethod`).
2. Для каждого параметра-объекта смотрит **тип-хинт**.
3. Через **`app()->make($class)`** строит граф зависимостей (рекурсивно).
4. Скаляры (`int $id`) подставляются из **route-параметров** по имени.

**Тонкости:**
- **Интерфейсы** нужно забиндить в `ServiceProvider::register()` — иначе контейнер не знает, какую реализацию подставить.
- **Контекстные биндинги**: `$this->app->when(UserController::class)->needs(Logger::class)->give(...)` — разные реализации для разных контроллеров.
- **Method injection** удобнее **`app()->make()` внутри метода** — DI явный, тестируется через моки в конструкторе.

**Подвох:** не вызывай `app()` / `resolve()` внутри методов — это **Service Locator**, скрытая зависимость. Всегда **type-hint** в сигнатуре.',
