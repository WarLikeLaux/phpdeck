    ...
    public static function all(): array
    {
        return [
            [
                ...
                'answer' => 'Nullable type (?Type, PHP 7.1+) - значит может быть Type или null. Union type (Type1|Type2, PHP 8.0+) - может быть любым из перечисленных. Глубокий разбор intersection, never и DNF - см. отдельные карточки этого топика; кратко: intersection (Type1&Type2, 8.1) - объект реализует ВСЕ перечисленные интерфейсы; never (8.1) - функция гарантированно не вернёт управление; DNF (8.2) - комбинация union и intersection в дизъюнктивной нормальной форме. mixed - любой тип.',
                'code_example' => '<?php
// Nullable
    ...
    public function __construct(
        ...
        public float $lng,
            ...
            [
                ...
                'answer' => 'К false приводятся: сам false, целые 0 и -0, числа с плавающей точкой 0.0 и -0.0, пустая строка "", строка "0" (именно из одного нолика), пустой массив, null и SimpleXML-объект, созданный из пустых тегов. Всё остальное — true, в том числе строки "false", "0.0", "00", "null" и массив с одним нулевым элементом. empty($x) при этом эквивалентно !(bool)$x (с поправкой на то, что empty не выдаёт warning на неопределённой переменной), поэтому empty("0.0") === false и (bool)"0.0" === true согласованы. Расхождение возникает не с empty, а с нестрогим равенством: "0.0" == 0 даёт true (числовое сравнение), хотя сама строка считается «истинной».',
                'difficulty' => 3,
                'topic' => 'php.types',
            ],
            [
                ...
                'answer' => 'PHP хранит float в формате IEEE 754 double, в двоичной системе. Многие десятичные дроби, такие как 0.1 и 0.2, в двоичном виде дают бесконечные периодические дроби, поэтому уже на этапе литерала теряется точность. В результате 0.1 + 0.2 даёт примерно 0.30000000000000004, и var_dump(0.1 + 0.2 == 0.3) выводит false. Корректное сравнение — через эпсилон, подобранный под предметную область: abs($a - $b) < $epsilon, где $epsilon зависит от величины операндов и числа операций. PHP_FLOAT_EPSILON (≈ 2.22e-16) подходит лишь для оценки одной элементарной операции — для накопленной ошибки берут существенно больший порог (например, 1e-9 для финансов в float, хотя там лучше вообще уйти на BCMath/целые).',
                'difficulty' => 3,
                'topic' => 'php.types',
            ],
            [
