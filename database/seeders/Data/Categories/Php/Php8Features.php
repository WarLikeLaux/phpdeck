...
// json_validate:            ~600 КБ peak memory',
            ...
            [
                ...
                'answer' => 'PHP 8.4 добавил четыре функции, заполняющие давнюю дыру в стандартной библиотеке: array_find, array_find_key, array_any, array_all. Все принимают $array и $callback, возвращают разное: array_find($arr, $cb) - первое значение, для которого callback вернул truthy, иначе null. array_find_key($arr, $cb) - ключ первого совпадения, иначе null. array_any($arr, $cb) - true, если хотя бы один элемент удовлетворяет, иначе false (аналог some/anyMatch); на пустом массиве возвращает false. array_all($arr, $cb) - true, если ВСЕ элементы удовлетворяют, иначе false (аналог every/allMatch); на пустом массиве возвращает true (vacuous truth). Главное преимущество - early termination: останавливаются на первом совпадении (или первом несовпадении для array_all), не проходят весь массив. До 8.4 для тех же задач писали array_filter() (но он проходит ВЕСЬ массив и аллоцирует промежуточный результат) или явный foreach с break. Эти функции - аналоги Array.find / .some / .every из JS, anyMatch / allMatch / findFirst из Java Streams.',
                'code_example' => '<?php
$users = [
...
$admin = $admin ? reset($admin) : null;',
            ...
            [
                ...
                'answer' => 'Подтверждённые в UPGRADING для 8.5: завершение case-веток через точку с запятой вместо двоеточия, продвижение PIE как рекомендованного инструмента установки расширений вместо PECL, точечные изменения в date/curl. Депрекации __sleep/__wakeup, обратных кавычек `cmd` и нестандартных имён приведений (boolean)/(integer)/(double) обсуждались/предлагались в RFC, но в финальный 8.5 в виде E_DEPRECATED не вошли — их статус следует уточнять по релизным заметкам.',
                'difficulty' => 3,
                'topic' => 'php.php8_features',
            ],
            [
...
$avatar = $user?->profile?->avatar;',
            ...
            [
                ...
                'answer' => 'Главные вехи. PHP 5.6 (2014) — последняя 5.x, добавила variadic ..., spread в вызовах, constant expressions. PHP 7.0 (2015) — переписан движок Zend Engine (Phpng): 2× скорость и 2× меньше памяти против 5.6, scalar type declarations (int/string), return type declarations, оператор ?? (null coalescing), <=> spaceship, anonymous classes, Throwable как корень Error/Exception. PHP 7.1 — nullable-типы (?int), void, iterable, multi-catch. PHP 7.2 — object type, тест-операторы Sodium, deprecation each(). PHP 7.3 — стабильные heredoc, list() с ключами. PHP 7.4 — typed properties (public int $age), arrow functions fn() =>, spread в массиве, FFI, preloading в opcache, ??= оператор, covariant return / contravariant param. PHP 8.0 (2020) — JIT, named arguments, attributes (#[Attribute]), constructor property promotion (public function __construct(public string $name) {}), match-выражение, nullsafe ?->, union types int|string, throw как выражение, weakmap, str_contains/str_starts_with/str_ends_with. PHP 8.1 — enums (включая backed), readonly properties, first-class callable syntax strlen(...), never type, intersection types A&B, new в инициализаторах, fibers (основа Async). PHP 8.2 — readonly classes, DNF-типы (A&B)|null, true/false/null как standalone типы, deprecation динамических свойств. PHP 8.3 — typed class constants, json_validate(), #[\Override] атрибут, readonly amendments. PHP 8.4 — property hooks (геттеры/сеттеры в синтаксисе свойств), asymmetric visibility (public/private(set)), новые методы массивов array_find/array_all/array_any.',
                'difficulty' => 3,
                'topic' => 'php.php8_features',
            ],
            [
                ...
                'answer' => 'Главные категории. 1) BC breaks из RFC — удалённые/изменённые функции (each, create_function, mb_strrpos с offset, money_format), изменённое поведение операторов (1 + "5 apples" в 8.0 кидает Warning, в 9.0 будет ошибка). 2) Строже типизация — внутренние функции стали выкидывать TypeError вместо тихих варнингов; raw-код типа strlen(null) теперь падает (PHP 8.1+ deprecation, 9.0 fatal). 3) Новые зарезервированные слова — readonly, enum, never, match как идентификаторы перестают работать. 4) Удаление расширений — mysql_* (5 → 7), ereg, mcrypt (7.2), oci8 как отдельная установка. 5) Поведение динамических свойств — в 8.2 deprecation, нужно #[AllowDynamicProperties] или стейтлесс DTO с явными свойствами. 6) Изменения в opcache/JIT — старые prefab-конфиги могут резко уронить производительность или съесть память; JIT в 8.0 в типовом веб-проде даёт ~0% и иногда крашит — обычно выключают (opcache.jit_buffer_size=0). 7) Зависимости — Composer-пакеты должны поддерживать новую версию (composer outdated / composer why-not). Очень часто застревают: симфони <5, doctrine <2.10, старые сторонние SDK. 8) Driver-несовместимость — pdo_mysql и mysqlnd ловят новые типы (BIGINT теперь int, не string в 8.1+). 9) Расширения PECL и opcache-shared-memory модули иногда отстают на месяцы. Стратегия миграции: rector php:81 + phpstan/psalm на максимум → CI с матрицей версий → канареечный деплой → обязательный e2e + smoke + load-тесты, потому что часть проблем (память, GC, JIT) видна только под нагрузкой.',
                'difficulty' => 3,
                'topic' => 'php.php8_features',
            ],
        ];
