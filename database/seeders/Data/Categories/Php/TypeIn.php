...
**Префикс `r`** — reverse (по убыванию). **Все эти функции мутируют** исходный массив (передан по ссылке).',
            ...
            [
                ...
                'answer' => '**`hash_equals($known_string, $user_string)`** — сравнение **за константное время** (constant-time): время не зависит от позиции первого несовпавшего байта.

**Зачем это нужно (timing attack):**
- Обычный `==` / `===` / `strcmp` для строк **выходит на первом отличии**.
- Атакующий по **микросекундам** разницы может посимвольно подобрать токен / HMAC / signature.
- `hash_equals` **всегда** обходит обе строки целиком.

**Где применять:**
- сравнение **HMAC** подписей вебхуков (Stripe, GitHub).
- сравнение **CSRF-токенов** в кастомной валидации.
- сравнение **password reset token** из БД и из ссылки.
- сравнение **API key** заголовка с эталоном.

**Тонкости:**
- Первый аргумент — **`$known_string`** (правильное значение из БД/конфига), второй — **`$user_string`** (то, что прислал пользователь). Длина первого считается «известной».
- Возвращает `false`, если **длины различаются** (но в константное время относительно `$known_string`).
- Для **паролей** используй `password_verify`, а не `hash_equals` — там ещё нужен salt.',
