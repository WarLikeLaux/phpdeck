    ...
    public static function all(): array
    {
        return [
            ...
            [
                ...
                'answer' => 'Имена переменных и ключи массивов в PHP регистрозависимы: $user и $User — это разные переменные. А вот имена функций, методов, классов, констант, объявленных через define() со вторым false-аргументом, и языковые конструкции (echo, if, true, null) — регистронезависимы, поэтому strlen и STRLEN вызовут одну и ту же функцию. Исключение составляют константы, объявленные через const или define() без флага case-insensitive (он удалён в PHP 8.0): они всегда регистрозависимы, и PHP_EOL не равно php_eol.',
                'difficulty' => 2,
                'topic' => 'php.basic_qa',
            ],
            [
            ...
            [
                ...
                'answer' => 'Базовый механизм — refcount: при достижении нуля zval освобождается мгновенно. Но циклы (объект A держит B, B держит A) refcount до нуля никогда не уменьшит. Поэтому PHP отслеживает «подозрительные» zval, чей refcount уменьшился, но остался положительным, и складывает их в корневой буфер. При переполнении буфера (по умолчанию zend.gc_root_buffer ~10000) запускается алгоритм Bacon-Rajan: обход в глубину, временное уменьшение refcount, всё, что обнулилось — мусор и удаляется, остальное восстанавливается. Управлять можно через gc_enable/gc_disable, gc_collect_cycles и смотреть статистику через gc_status.',
                'difficulty' => 4,
                'topic' => 'php.basic_qa',
            ],
            [
