...
$config["timeout"] ??= 30; // если не задано, поставить 30',
            ...
            [
                ...
                'answer' => 'Оператор <=> (PHP 7+) выполняет трёхстороннее сравнение: возвращает -1 если левый меньше, 0 если равны, 1 если левый больше. Применяет те же правила, что и обычные операторы сравнения. Главный кейс — callback для usort: заменяет громоздкие if/else или приём «$a - $b», который ломается на больших значениях из-за переполнения int. Работает и для строк (по словарю), и для массивов (поэлементно).',
                'code_example' => '<?php
echo 1 <=> 2;   // -1
echo 2 <=> 2;   //  0
echo 3 <=> 2;   //  1
...
var_dump(array_key_exists("key", $arr)); // true',
            ...
            [
                ...
                'answer' => 'Стандартный набор: + - * / для сложения, вычитания, умножения и деления; % — остаток от деления; ** — возведение в степень (PHP 5.6+). Оператор / всегда возвращает float, если результат не целое: 7 / 2 даёт 3.5, а 6 / 2 даёт int(3). Если нужно гарантированно целочисленное деление с отбрасыванием дробной части, используйте intdiv(7, 2) — вернёт 3. Деление на 0 для целых даёт DivisionByZeroError (PHP 8+), для float — INF/NAN.',
                'code_example' => '<?php
echo 7 / 2;        // 3.5  (float)
...
echo 2 ** 10;      // 1024 (степень)',
            ...
            [
                ...
                'answer' => 'Стандартные: && (И), || (ИЛИ), ! (НЕ). Они вычисляются «лениво»: $a && $b не вычислит $b, если $a уже false; $a || $b не вычислит $b, если $a уже true. Есть также текстовые аналоги and, or, xor, но у них НИЖЕ приоритет, чем у =. Поэтому $x = $a or $b на самом деле работает как ($x = $a) or $b — $x получает только значение $a. Хорошая практика — использовать && и || и не путать с and/or.',
                'code_example' => '<?php
$x = false || true;   // true
