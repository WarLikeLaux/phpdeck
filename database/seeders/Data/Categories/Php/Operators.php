...
| Оператор | Что делает |
|---|---|
| **`==`** | равно **с приведением типов** (loose) |
| **`===`** | равно **строго**: тип И значение |
| **`!=`** / **`<>`** | не равно (loose) |
| **`!==`** | не равно строго |
| **`<`**, **`>`**, **`<=`**, **`>=`** | сравнение по величине |
| **`<=>`** (PHP 7+) | spaceship: возвращает **`-1`** / **`0`** / **`1`** — для `usort` |

**Главный подвох `==`** — неявное приведение типов:
- `"0" == false` → **`true`**
- `null == 0` → **`true`**
- `null == ""` → **`true`**
- `"1" == "01"` → **`true`** (обе строки приводятся к `int`)
- `"10" == "1e1"` → **`true`** (научная форма)

**Изменения в PHP 8:**
- `0 == "abc"` → **`false`** (раньше было `true`, потому что `"abc"` приводилось к `0`).
- Сравнение **число + non-numeric строка** теперь сравнивает строки **как строки**.

**Правило:** **всегда используй `===`**, кроме случаев, где осознанно нужен loose-compare (например, "0" из `$_POST`).

**Подвох `in_array` и `array_search`:** по умолчанию `==`, в горячем коде передавай **третий аргумент `true`** для строгого сравнения.',
                'code_example' => '<?php
...
$name = $user["name"] ?? "Guest";',
            ...
            [
                ...
                'answer' => 'Два **похожих по виду**, но **разных по смыслу** оператора:

| Оператор | Когда срабатывает «дефолт» | Семантика |
|---|---|---|
| **`??`** (null coalescing, PHP 7+) | значение **`null`** или переменная **не определена** | защита от `null`/undefined |
| **`?:`** (Elvis / ternary shortcut) | значение **falsy**: `null`, `0`, `""`, `"0"`, `false`, `[]` | защита от «любой пустоты» |

**Главная разница:** `??` различает `0` / `""` / `false` от `null` — **`?:` нет**.

**Примеры на одинаковых данных:**
```
$a = null; $b = ""; $c = 0;
...
**Бонусы `??`:**
- **`$config["timeout"] ?? 30`** — не выдаёт Warning «Undefined index», в отличие от `?:`.
- **`??=`** (PHP 7.4+) — присваивание с дефолтом: `$config["timeout"] ??= 30;`.
- **Цепочки**: `$a ?? $b ?? $c ?? "default"` — берётся первое не-null.
- **С `throw` (PHP 8+)**: `$user = $repo->find($id) ?? throw new NotFoundException();`.

**Когда брать `?:`:**
- осознанно нужен «любой falsy = default» (например, пустая строка = guest).
- НЕ работает с несуществующими ключами без `@`/`isset`.',
                'code_example' => '<?php
$a = null;
$b = "";
$c = 0;
...
$config["timeout"] ??= 30; // если не задано, поставить 30',
            ...
            [
                ...
                'answer' => '**Конвейерный оператор `|>`** (PHP **8.5**) передаёт результат **левого** выражения как **первый аргумент** правому **callable**, позволяя выстраивать цепочки преобразований **слева направо**.

**Зачем нужно:**
- Альтернатива вложенным вызовам типа `ucfirst(strtolower(trim($s)))` — читается **справа налево**, мозг устаёт.
- Похоже на **F#** / **Elixir** / **Hack** `|>`.
- Хорошо сочетается с **first-class callable syntax** (`trim(...)`).

**Особенности:**
- Правая часть **обязана быть `callable`** — функция, замыкание, invokable, метод (`$obj->method(...)`).
- Передаётся **только один** аргумент (результат левой части). Для частичного применения — оборачивай в `fn($x) => f($x, $extra)`.
- Ассоциативность **слева направо**: `$a |> f(...) |> g(...)` это `g(f($a))`.
- Тип-чек статанализом: PHPStan/Psalm видят сигнатуру и проверяют.

**Когда осмысленно:**
- **Data pipelines** — нормализация строк, обработка коллекций.
- Композиция **чистых функций**.

**Когда не стоит:** ради `|>` не превращай тривиальные `f($x)` в одношаговый пайп — это уже снижение читаемости.',
                'code_example' => '<?php
