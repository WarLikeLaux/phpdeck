...
// [1, 2, 3]',
            ...
            [
                ...
                'answer' => '**Обычное замыкание** внутри метода класса **неявно захватывает `$this`** — текущий объект и его scope.
- внутри closure доступны **`$this->property`** и **приватные** члены класса
- closure **удерживает** объект — пока closure жив, `$this` **не освободится** GC

**Static-замыкание** (`static function() use (...) {...}` или `static fn() => ...`):
- **НЕ привязывается** к `$this`
- **НЕ привязывается** к scope создавшего класса
- попытка `$this` внутри → **`Error: Using $this when not in object context`**

**Зачем нужен `static` — три причины:**

| Причина | Что даёт |
| --- | --- |
| **Утечки памяти** | если closure хранится долго (queue, cache, длинная коллекция) — не-static версия **держит объект-родителя** в памяти; `static` освобождает сразу |
| **Безопасность scope** | если closure уходит в **чужой код** (плагин, listener) — `static` гарантирует, что внутри не утечёт состояние объекта (нельзя через рефлексию вытащить `$this`) |
| **Семантическая ясность** | если функция **не нуждается** в объекте — `static` это документирует |

**Влияние на производительность:**

| Аспект | Эффект |
| --- | --- |
| **Скорость вызова** | разница **микроскопическая** (десятки нс) — JIT и OPcache всё равно оптимизируют |
| **Память** | **заметная** экономия, когда closures массово создаются/хранятся (Laravel pipeline, event handlers, generators) |
| **Работа GC** | меньше циклических ссылок → реже срабатывает циклический GC |

**Привязка через `Closure::bind` / `bindTo`:**
- для **обычного** closure: `bindTo($newThis, $scope)` меняет `$this`
- для **`static`** closure: можно только **сменить scope** (`$newThis = null` обязательно), `$this` нельзя привязать в принципе
- именно через `static` говорят «**closure без контекста**, можно безопасно сериализовать/передавать»',
                'code_example' => '<?php
class Service {
    public function __construct(private Logger $logger) {}

    public function tasks(): array {
        // ❌ Утечёт $this в очередь — Service не освободится, пока closure жив
        $a = function(int $x) {
            $this->logger->log($x);
            return $x * 2;
        };
...
// $this в static closure — Error
$c = static fn() => $this->x;   // Error при вызове

// static + bindTo может только менять scope, не $this
$cb = static fn() => 42;
$cb2 = Closure::bind($cb, null, SomeClass::class); // OK, $this=null
// $cb3 = Closure::bind($cb, new SomeClass);       // Error — static не привязать к объекту',
                'code_language' => 'php',
                'difficulty' => 4,
                'topic' => 'php.closures',
            ],
