    ...
    public function tasks(): array {
        ...
        return [$a, $b];
            ...
            [
                ...
                'answer' => 'Функция без имени, которую можно положить в переменную или передать как аргумент (объект класса Closure). Удобно для коротких callback-ов «на лету» — например, в array_map, usort, фильтрах коллекций — без объявления отдельной именованной функции. Может захватывать переменные окружения через use ($var). Короткая форма (PHP 7.4+) — arrow function: fn($x) => $x * 2.',
                'code_example' => '<?php
