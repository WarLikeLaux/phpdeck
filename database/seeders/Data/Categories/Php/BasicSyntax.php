...
echo \'Привет, $name\'; // Привет, $name',
            ...
            [
                'category' => 'PHP',
                'question' => 'Что такое сравнение по модулю (%) в PHP и где его обычно применяют?',
                'answer' => 'Оператор % возвращает остаток от деления первого операнда на второй: 7 % 3 даёт 1, 10 % 2 даёт 0. Чаще всего применяется для проверки чётности (n % 2 === 0), для разбиения списка на группы по N через шаги, для пагинации, FizzBuzz и для вычисления «каждый k-й». Знак результата в PHP совпадает со знаком левого операнда: -7 % 3 даёт -1.',
                'code_example' => 'for ($i = 1; $i <= 10; $i++) {
    if ($i % 2 === 0) {
        echo "$i — чётное\n";
    }
}',
